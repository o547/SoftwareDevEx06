    ...
    def shortest_route_search(self, request, start, goal):
        try:
            if RouteSearchProcess.graph_instance is None:
                ...
                filtered_nodes = [
                    node for node in nodes if "区画調整" not in node["section_name"]
                ]
...
class ChatBotProcess:
    ...
    def create_prompt(self, request, user_input):
        ...
        prompt = (
            ...
            "区画名は括弧や空白なども含めて、情報一覧の正式名称と正確に一致させてください"
            "ユーザの入力の例：609教室から中丸研に行きたい"
            "出力の例：609教室からカフェテリアまでの経路案内はこちらです！<br><a href=/search/教室棟_6階_609教室（PC）/研究棟_14階_中丸智貴研究室>/search/教室棟_6階_609教室（PC）/研究棟_14階_中丸智貴研究室</a>"
